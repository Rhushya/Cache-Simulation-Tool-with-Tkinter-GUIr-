        ...
        if self.mapping_technique == "direct":
            if self.cache[index] == tag:
                ...
                if self.cache[index] != -1:
                    self.evictions += 1
                    output_text.insert(
                        tk.END, f"Cache eviction for address {address}\n"
                    )
            ...
            if tag in self.cache:
                ...
                if len(self.cache) < self.cache_size // self.block_size:
                    ...
                    output_text.insert(
                        tk.END, f"Cache miss for address {address}, tag {tag} added\n"
                    )
                else:
                    self.evictions += 1
                    output_text.insert(
...
combobox_mapping_technique = ttk.Combobox(
    root, values=["Direct", "Set Associative", "Associative"], width=15
)
combobox_mapping_technique.grid(row=3, column=1, padx=5, pady=5)
combobox_mapping_technique.current(0)

# Memory trace
label_memory_trace = ttk.Label(
    root, text="Memory Trace (addresses separated by space):"
)
